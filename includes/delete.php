...
if($delete){
  ...
  $log = fopen('../Back-Office/log_admin.txt', 'a+');
  ...
  fputs($log, $line);
  fclose($log);
  header('location: ../Back-Office/db_'.$_GET['table'].'.php?message=Supression réussite.&type=success');
} else {
  header('location: ../Back-Office/db_'.$_GET['table'].'.php?message=Erreur lors de la suppression.&type=danger');
}
?>
