...
if (!isset($_SESSION['email'])) {
	include("../Page_non_connecte.php");
		...
		$s = 'SELECT id,nom,email,image,statut
					FROM admin';
		$req_select = $db->query($s);
		$select = $req_select->fetchAll(PDO::FETCH_ASSOC);

		foreach ($select as $admin) {
			$date = date_create($admin["email"]);
			echo '<tr>
						 <td scope="row">(' . $admin['id'] .')</td>
						 <td scope="row">' . $admin['nom'] .'</td>
						 <td scope="row">' . $admin['email'] .'</td>
						 <td scope="row">' . $admin['image'] .'</td>
						 <td scope="row">' . $admin['statut'] .'</td>
				<td scope="row" id="modif">

<form action="modif-element.php?element=admin&id='.$admin['id'].'" method="post">
				<button type="button" class="btn btn-info btn-circle btn">Modifier<i class="glyphicon glyphicon-ok"></i></button>
					</form>
				</td>
							<td scope="row">
 						<form action="delete.php?table=admin&id='.$admin['id'].'" method="post">
