...
<html lang="en">
  <head>
    ...
    <title>Information forfait</title>
  </head>
  <style type="text/css">
  	.navbar.bg-light{
  ...
  <body>
  	...
  	include("../includes/db_connexion.php");
		...
		$s = 'SELECT id,image,nom,description,add_date,add_admin
					FROM Forfaits';
		$req_select = $db->query($s);
		$select = $req_select->fetchAll(PDO::FETCH_ASSOC);

		foreach ($select as $Forfaits) {
			$date = date_create($Forfaits["nom"]);
			echo '<tr>
						 <td scope="row">(' . $Forfaits['id'] .')</td>
						 <td scope="row">' . $Forfaits['image'] .'</td>
						 <td scope="row">' . $Forfaits['nom'] .'</td>
						 <td scope="row">' . $Forfaits['description'] .'</td>
						 <td scope="row">'. $Forfaits['add_date'] .'</td>
						 <td scope="row">' . $Forfaits['add_admin'] .'</td>
						 <form action="../includes/modif-element.php?element=Article" method="post">
						  ...
						  <td scope="row">
						  <form action="delete.php?table=Forfaits&id='.$Forfaits['id'].'" method="post">
