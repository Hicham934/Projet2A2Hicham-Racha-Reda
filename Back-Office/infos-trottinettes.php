...
<html lang="en">
  ...
  <body>
  	...
  	include("../includes/db_connexion.php");
		...
		$s = 'SELECT id,Nom_complet,image1,image2,prix,stock,add_date,add_admin
					FROM Catalogue_trottinette';
		$req_select = $db->query($s);
		$select = $req_select->fetchAll(PDO::FETCH_ASSOC);

		foreach ($select as $Catalogue_trottinette) {
			$date = date_create($Catalogue_trottinette["Nom_complet"]);
			echo '<tr>
						 <td scope="row">(' . $Catalogue_trottinette['id'] .')</td>
						 <td scope="row">' . $Catalogue_trottinette['Nom_complet'] .'</td>
						 <td scope="row">' . $Catalogue_trottinette['image1'] .'</td>
						 <td scope="row">' . $Catalogue_trottinette['image2'] .'</td>
						 <td scope="row">' . $Catalogue_trottinette['prix'] .'</td>
						  <td scope="row">' . $Catalogue_trottinette['stock'] .'</td>
						 <td scope="row">'. $Catalogue_trottinette['add_date'] .'</td>
						 <td scope="row">' . $Catalogue_trottinette['add_admin'] .'</td>
						 <form action="../includes/modif-element.php?element=Article" method="post">
						  <td scope="row"><button type="button" class="btn btn-info btn-circle btn">Modifier<i class="glyphicon glyphicon-ok"></i></button></td>
						  </form>
						  <td scope="row">
						  <form action="delete.php?table=Catalogue_trottinette&id='.$Catalogue_trottinette['id'].'" method="post">
